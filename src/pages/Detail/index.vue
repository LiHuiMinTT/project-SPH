<template>
  <div class="detail">
    ...
    <section class="con">
      <!-- 导航路径区域 -->
      <div class="conPoin">
        <span v-show="categoryView">{{ categoryView }}</span>
      </div>
      <!-- 主要内容区域 -->
      <div class="mainCon">
        <!-- 左侧放大镜区域 -->
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in saleAttr" :key="index">
                ...
                <dd
                  ...
                  :key="attr.id"
                  @click="changeActive(attr, item.spuSaleAttrValueList)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a href="javascript:" class="mins" @click="decreaNum">-</a>
              </div>
              <div class="add">
                <a @click="updataCart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...mapState("detailModule", ["detailInfo"]),
    ...mapGetters("detailModule", [
      "categoryView",
      "price",
      "saleAttr",
      "skuInfo",
  ...
  mounted() {
    this.$store.dispatch("detailModule/getDetail", this.$route.params.skuid);
  },
  methods: {
     async updataCart(){
       try {
           await this.$store.dispatch('detailModule/updataCart',{skuId:this.$route.params.skuid,skuNum:this.purchaseNum});
           this.$router.push({
             name:'addShopcart',
             params:{
               skuId:this.$route.params.skuid,
               skuNum:this.purchaseNum
             }
           });
    ...
    addNum() {
      this.purchaseNum++;
      this.$refs.input.value = this.purchaseNum;
    },
    decreaNum() {
    ...
    check(event) {
      ...
      this.purchaseNum = event.target.value;
    },
    changeActive(attr, attrList) {
      // const a = {
      //   valueOf:function(){return '123546'},
      //   toString:function(){
      //     return '456';
      //   }
      // };
