  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        ...
        <ul
          class="list clearFix"
          v-for="item in payinfo.detailArrayList"
          :key="(u = item.skuId)"
        >
          <li>
    ...
    <div class="trade">
      <div class="price">
        应付金额:　<span>{{ payinfo.totalAmount }}</span>
      </div>
...
export default {
  ...
  methods: {
    async getInfo() {
      try {
        await this.$store.dispatch("payModule/getPayInfo");
      } catch (error) {
        alert("加载失败");
      }
    },
    async goPay() {
      ...
      const result = await this.$API.reqPayInfo(this.payinfo.tradeNo,data);
      if(result.code==200){
        this.$router.push({
          path:'/pay',
          name:'pay',
